...
class DiamonSquare(object):
    ...
    def generate(self):
        ...
        while step_size > 1:
            ...
            for x in range(0, len(grid) - 1, step_size):
                for y in range(0, len(grid[x]) - 1, step_size):
                    self.diamond_step(x, y, step_size)

            even = True
            for x in range(0, len(grid) - 1, hstep):
                y_start = 0 if even else hstep
                for y in range(y_start, len(grid[x]) - 1, hstep):
